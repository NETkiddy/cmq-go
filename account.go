...
func (this *Account) CreateQueue(queueName string, queueMeta QueueMeta) (err error) {
	...
	_, err = doCall(this.client, param, "CreateQueue")
	if err != nil {
...
func (this *Account) DeleteQueue(queueName string) (err error) {
	param := make(map[string]string)
	if queueName == "" {
	...
	_, err = doCall(this.client, param, "DeleteQueue")
	if err != nil {
...
func (this *Account) ListQueue(searchWord string, offset, limit int) (totalCount int, queueList []string, err error) {
	...
	resMap, err := doCall(this.client, param, "ListQueue")
	if err != nil {
...
func (this *Account) CreateTopic(topicName string, maxMsgSize int) (err error) {
	_createTopic(this.client, topicName, maxMsgSize, 1)
	return
}
...
func _createTopic(client *CMQClient, topicName string, maxMsgSize, filterType int) (err error) {
	...
	_, err = doCall(client, param, "CreateTopic")
	if err != nil {
...
func (this *Account) DeleteTopic(topicName string) (err error) {
	param := make(map[string]string)
	if topicName == "" {
	...
	_, err = doCall(this.client, param, "DeleteTopic")
	if err != nil {
...
func (this *Account) ListTopic(searchWord string, offset, limit int) (totalCount int, topicList []string, err error) {
	...
	resMap, err := doCall(this.client, param, "ListTopic")
	if err != nil {
...
func (this *Account) CreateSubscribe(topicName, subscriptionName, endpoint, protocol, notifyContentFormat string) (err error) {

	return _createSubscribe(this.client, topicName, subscriptionName, endpoint, protocol, nil, nil, NotifyStrategyDefault, notifyContentFormat)
}

func _createSubscribe(client *CMQClient, topicName, subscriptionName, endpoint, protocol string, filterTag []string, bindingKey []string, notifyStrategy, notifyContentFormat string) (err error) {
	param := make(map[string]string)
	if topicName == "" {
	...
	_, err = doCall(client, param, "Subscribe")
	if err != nil {
...
func (this *Account) DeleteSubscribe(topicName, subscriptionName string) (err error) {
	...
	_, err = doCall(this.client, param, "Unsubscribe")
	if err != nil {
...
func doCall(client *CMQClient, param map[string]string, opration string) (resMap map[string]interface{}, err error) {
